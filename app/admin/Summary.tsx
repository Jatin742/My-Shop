...
const Summary:React.FC<SummaryProps> = ({ orders, products, users }) => {
    ...
    const summaKeys = Object.keys(summaryData);
    ...
    return ( 
        <div className="max-w-[1150px] m-auto">
            ...
            <div className="grid grid-cols-2 gap-3 max-h-[50vh] overflow-y-auto">
                {
                    summaKeys && summaKeys.map((key) =>{
                        return <div key={key} className="rounded-xl border-2 p-4 flex flex-col items-center gap-2 transition">
                            <div className="text-xl md:text-4xl font-bold">
                                {
                                    summaryData[key].label === 'Total Sale' ? <>{formatNumber(summaryData[key].digit)}</> : <>{formatNumber(summaryData[key].digit)}</>
                                }
                            </div>
